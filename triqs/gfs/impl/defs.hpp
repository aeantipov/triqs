...
namespace gfs {
 ...
 template <typename Mesh> struct gf_default_target { using type = matrix_valued; };
 template <typename M> using gf_default_target_t = typename gf_default_target<M>::type;

 template <typename Mesh, typename Target> struct gf_default_singularity { using type = nothing; };
 template <typename M, typename T> using gf_default_singularity_t = typename gf_default_singularity<M, T>::type;
 ...
 template <typename M, typename T, typename S, typename E> struct is_gf_or_view<gf_view<M, T, S, E>, void> : std::true_type {};
 template <typename M, typename T, typename S, typename E> struct is_gf_or_view<gf_view<M, T, S, E>, M> : std::true_type {};

 template <typename M, typename T, typename S, typename E>
 struct is_gf_or_view<gf_const_view<M, T, S, E>, void> : std::true_type {};

 template <typename M, typename T, typename S, typename E> struct is_gf_or_view<gf_const_view<M, T, S, E>, M> : std::true_type {};
 ...
 template <typename Mesh, typename Target, typename Singularity> struct gf_symmetry { using type = nothing; };

 /*----------------------------------------------------------
  *  indices
  *--------------------------------------------------------*/

 template <typename Target> struct gf_indices { using type = nothing; };

 template <> struct gf_indices<matrix_valued> { using type = gf_indices_tuple<2>; };
 template <> struct gf_indices<tensor_valued<3>> { using type = gf_indices_tuple<3>; };
 template <> struct gf_indices<tensor_valued<4>> { using type = gf_indices_tuple<4>; };
 ...
 template <typename M, typename T, typename S, typename E> auto get_gf_data_shape(gf<M, T, S, E> const &g) {
  return get_shape(g.data());
 }

 template <typename M, typename T, typename S, typename E> auto get_gf_data_shape(gf_view<M, T, S, E> const &g) {
  return get_shape(g.data());
 }

 template <typename M, typename T, typename S, typename E> auto get_gf_data_shape(gf_const_view<M, T, S, E> const &g) {
  return get_shape(g.data());
 }

 template <typename M, typename T, typename S, typename E> auto get_target_shape(gf<M, T, S, E> const &g) {
  return g.data().shape().front_pop();
 }

 template <typename M, typename T, typename S, typename E> auto get_target_shape(gf_view<M, T, S, E> const &g) {
  return g.data().shape().front_pop();
 }

 template <typename M, typename T, typename S, typename E> auto get_target_shape(gf_const_view<M, T, S, E> const &g) {
  return g.data().shape().front_pop();
 }
